...
const containsW = (str) => {
  const testString = /w/;
  const result = testString.test(str);
  return result;
};
...
const isNum = (input) => {
  const testForNum = /\d/gm;
  const result = testForNum.test(input);
  return result;
};
...
const containsWorld = (input) => {
  const testForWorld = /world/gm;
  const result = testForWorld.test(input);
  return result;
};
...
const isCapitalized = (str) => {
  const testForCap = /[A-Z][a-zA-Z]*/gm;
  const result = str.match(testForCap);
  return result || [];
};
...
const citiesAtoJ = (arr) => {
  const newArray = [];
  arr.forEach(city => {
    const testCities = city.match(/^[a-jA-J]\w+/gm);
    if (testCities){
      newArray.push(testCities[0]);
    }
  });
  return newArray;
};
...
const findShells = (str) => {
  const newShell = str.match(/\w+ells/gm);
  return newShell;
};
